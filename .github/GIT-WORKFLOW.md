Git workflow for yii2-dropdown-x contributors
=========================================

So you want to contribute to yii2-dropdown-x? Great! But to increase the chances of your changes being accepted quickly, please
...
### 1. [Fork](http://help.github.com/fork-a-repo/) the yii2-dropdown-x repository on GitHub and clone your fork to your development environment 

```
git clone git@github.com:YOUR-GITHUB-USERNAME/yii2-dropdown-x.git
```
...
### 2. Add the main yii2-dropdown-x repository as an additional git remote called "upstream"

Change to the directory where you cloned yii2-dropdown-x, normally, "yii2-dropdown-x". Then enter the following command:

```
git remote add upstream git://github.com/kartik-v/yii2-dropdown-x.git
```

### 3. Prepare the testing environment

- You should have a working yii 2 development environment in which you have already installed `yii2-dropdown-x` and includes latest and updated `yii2-dropdown-x` fork from source.
- Ensure you have the latest `dev-master` releases of all dependent extensions via your composer updates
- Ensure you use the above cloned latest `yii2-dropdown-x` code in your testing environment

**Now you have a working playground for hacking on yii2-dropdown-x.**
...
### 2. Fetch the latest code from the main yii2-dropdown-x branch
...
### 3. Create a new branch for your feature based on the current yii2-dropdown-x master branch
...
### 7. Pull the latest yii2-dropdown-x code from upstream into your branch
...
you should fix them now and commit the changes again. This ensures that it's easy for the yii2-dropdown-x team to merge your changes
with one click.
...
and become part of the next yii2-dropdown-x release. If not, don't be disheartened, different people need different features and yii2-dropdown-x
